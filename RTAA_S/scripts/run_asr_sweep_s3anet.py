"""ASR sweep on a fourth architecture — S3ANet / PaviaU (TGRS 2024, explicitly designed to defend against adversarial attacks — a stronger test than SACNet).
...
Structural difference from the pixel/patch targets: S3ANet, like SACNet, is a whole-scene
...
(the whole scene is perturbed every time, as S3ANet's architecture
...
CHECKPOINT = "checkpoints/s3anet_paviau.pt"
RTM_CHECKPOINT = "checkpoints/rtm_surrogate_103bands.pt"
DATA_DIR = Path(S3ANET_REPO_DIR) / "Data" / "PaviaU"
N_CLASSES = 9
...
def main() -> None:
    ...
    model = load_s3anet(n_bands=n_bands, n_classes=N_CLASSES, device=str(device))
    ...
    with open("asr_sweep_s3anet_results.json", "w") as f:
    ...
    print("\nSaved per-seed results to asr_sweep_s3anet_results.json")
