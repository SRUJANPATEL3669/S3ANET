...
def run_ifgsm(dataID, args):
    ...
    # ------ I-FGSM Attack ------
    adv_image = images.clone().detach()
    label_tar = torch.from_numpy(Y_tar).long().cuda()

    alpha = args.alpha if args.alpha else args.epsilon / args.iters
    ...
    for i in range(args.iters):
        ...
        adv_noise = alpha * adv_image.grad.data.sign()
        adv_image = adv_image - adv_noise
        
        # Clip to epsilon ball around original image
        eta = torch.clamp(adv_image - images, min=-args.epsilon, max=args.epsilon)
        adv_image = torch.clamp(images + eta, 0, 1).detach()
    ...
    plt.imsave(save_path_prefix + 'S3ANet_IFGSM_OA' + repr(int(OA*10000)) +
...
def main(args):
    ...
    for did in dataset_ids:
        print('\n' + '='*55)
        print('  I-FGSM on Dataset: %s' % DataName[did])
        print('='*55)
        try:
            row = run_ifgsm(did, args)
            results.append(row)
    ...
    if results:
        ...
        excel_path = args.save_path_prefix + 'IFGSM_All_Metrics_Results.xlsx'
...
if __name__ == '__main__':
    ...
    parser.add_argument('--alpha',   type=float, default=0.005) # 0 means epsilon/iters
