...
# ── Cell: Generate train/test splits for S3ANet/SACNet ──
cells.append(_code([
    ...
    "\n",
    "    # Convert 1-indexed class labels to 0-indexed (CrossEntropy2d expects 0..N-1)\n",
    "    # Background pixels (label 0) become 255 (CrossEntropy2d ignore label)\n",
    "    Y_save = Y.copy()\n",
    "    mask_bg = (Y_save == 0)\n",
    "    Y_save[~mask_bg] -= 1\n",
    "    Y_save[mask_bg] = 255\n",
    ...
    "    np.save(os.path.join(out_dir, 'Y.npy'), Y_save)\n",
...
cells.append(_code([
    ...
    "    Y_original = np.load(data_dir + 'Y_original.npy')\n",
    ...
    "    OA, kappa, _ = CalAccuracy(predict[test_array] + 1, Y_original[test_array])\n",
...
cells.append(_code([
    ...
    "            scene_norm = cube_norm.transpose(2, 0, 1)  # (n_bands, H, W) — global norm OK for baselines\n",
    "            # But for S3ANet/SACNet which were trained with per-band normalization, re-normalize\n",
    "            if model_name in ('S3ANet', 'SACNet'):\n",
    ...
    "                scene_norm = flat_norm_scene.reshape(h, w, n_bands).transpose(2, 0, 1)\n",
