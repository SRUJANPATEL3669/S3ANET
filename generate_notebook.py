"""Generate the full adversarial benchmark Colab notebook.

Run this script to regenerate notebooks/adversarial_benchmark_colab.ipynb.
"""
import json
import os
...
# ── Cell: Mount Drive ──
cells.append(_md(["## 0. Mount Google Drive & Set Paths"]))
cells.append(_code([
...
cells.append(_code([
    ...
    "!git clone https://github.com/danfenghong/IEEE_TGRS_SpectralFormer.git /content/SpectralFormer 2>/dev/null || echo 'SpectralFormer already cloned'\n",
    "\n",
    "# Set environment variables for repo paths\n",
    "os.environ['S3ANET_REPO_DIR'] = '/content/S3ANet'\n",
    "os.environ['SACNET_REPO_DIR'] = '/content/SACNet'\n",
    "os.environ['SPECTRALFORMER_REPO_DIR'] = '/content/SpectralFormer'\n",
...
cells.append(_md(["## 2. Prepare Data & Dataset Splits"]))
cells.append(_code([
    "import numpy as np\n",
    "import shutil\n",
    "\n",
    ...
    "}\n",
    "\n",
    "# Symlink .mat data files into S3ANet and SACNet Data directories\n",
    "for repo in ['/content/S3ANet', '/content/SACNet']:\n",
    "    for ds_name, ds_info in DATASETS.items():\n",
    "        data_path = f'{repo}/Data/{ds_name}'\n",
    "        os.makedirs(data_path, exist_ok=True)\n",
    "        # Copy .mat files\n",
    "        for f in [ds_info['data_file'], ds_info['gt_file']]:\n",
    "            src = os.path.join(DATA_DIR, f)\n",
    "            dst = os.path.join(data_path, f)\n",
    "            if os.path.exists(src) and not os.path.exists(dst):\n",
    "                os.symlink(src, dst)\n",
    "                print(f'  Linked {src} -> {dst}')\n",
    "\n",
    "print('Data files linked.')\n",
]))

# ── Cell: GenSample for S3ANet/SACNet ──
cells.append(_code([
    "# Generate train/test splits for S3ANet and SACNet repos\n",
    "# dataID mapping: 1=PaviaU, 2=IndianPines (may vary by repo)\n",
    "# We run GenSample.py for each dataset that has data available\n",
    "\n",
    "import subprocess\n",
    "\n",
    "for repo_name, repo_path in [('S3ANet', '/content/S3ANet'), ('SACNet', '/content/SACNet')]:\n",
    "    gen_script = os.path.join(repo_path, 'GenSample.py')\n",
    "    if os.path.exists(gen_script):\n",
    "        for ds_name in DATASETS:\n",
    "            # Check if split files already exist\n",
    "            split_dir = os.path.join(repo_path, 'Data', ds_name)\n",
    "            if os.path.exists(os.path.join(split_dir, 'X.npy')):\n",
    "                print(f'  {repo_name}/{ds_name}: splits already exist, skipping.')\n",
    "                continue\n",
    "            try:\n",
    "                result = subprocess.run(\n",
    "                    ['python', gen_script, '--dataID', '1', '--train_samples', '300'],\n",
    "                    cwd=repo_path, capture_output=True, text=True, timeout=120\n",
    "                )\n",
    "                if result.returncode == 0:\n",
    "                    print(f'  {repo_name}/{ds_name}: splits generated.')\n",
    "                else:\n",
    "                    print(f'  {repo_name}/{ds_name}: GenSample.py failed: {result.stderr[:200]}')\n",
    "            except Exception as e:\n",
    "                print(f'  {repo_name}/{ds_name}: Error running GenSample.py: {e}')\n",
    "    else:\n",
    "        print(f'  {repo_name}: GenSample.py not found at {gen_script}')\n",
    "\n",
    "print('Dataset split generation complete.')\n",
]))

# ── Cell: Imports ──
cells.append(_md(["## 3. Core Imports & Utilities"]))
cells.append(_code([
    "import time\n",
    "import json\n",
    "import torch\n",
    ...
    "        sid_pq = (p * np.log(p / (q + 1e-12) + 1e-12)).sum(axis=1)\n",
    "        sid_qp = (q * np.log(q / (p + 1e-12) + 1e-12)).sum(axis=1)\n",
...
# ── Cell: Train HybridSN (patchwise) ──
cells.append(_md(["## 4. Train Models\n", "### 4.1 HybridSN (Patchwise — 3D-2D CNN)"]))
cells.append(_code([
    ...
    "    ckpt_path = os.path.join(CHECKPOINT_DIR, f'hybridsn_patch_{ds_name.lower()}.pt')\n",
...
# ── Cell: Train HybridSN pixelwise ──
cells.append(_md(["### 4.2 HybridSN (Pixelwise — 1D Spectral CNN)"]))
cells.append(_code([
    ...
    "    ckpt_path = os.path.join(CHECKPOINT_DIR, f'hybridsn_pixel_{ds_name.lower()}.pt')\n",
    "    cube, labels = load_mat_dataset(ds_name)\n",
    "    if os.path.exists(ckpt_path):\n",
    "        print(f'  Loading checkpoint from {ckpt_path}')\n",
...
# ── Cell: Train SpectralFormer ──
cells.append(_md(["### 4.3 SpectralFormer (Patchwise / CAF)"]))
cells.append(_code([
    ...
    "    ckpt_path = os.path.join(CHECKPOINT_DIR, f'spectralformer_patch_{ds_name.lower()}.pt')\n",
    "    cube, labels = load_mat_dataset(ds_name)\n",
    "    if os.path.exists(ckpt_path):\n",
    "        print(f'  Loading checkpoint from {ckpt_path}')\n",
    "        config = dict(_VARIANT_CONFIGS['patchwise'])\n",
...
cells.append(_code([
    ...
    "    ckpt_path = os.path.join(CHECKPOINT_DIR, f's3anet_{ds_name.lower()}.pt')\n",
    "    if os.path.exists(ckpt_path):\n",
    "        print(f'  Loading checkpoint from {ckpt_path}')\n",
...
cells.append(_code([
    ...
    "    ckpt_path = os.path.join(CHECKPOINT_DIR, f'sacnet_{ds_name.lower()}.pt')\n",
    "    if os.path.exists(ckpt_path):\n",
    "        print(f'  Loading checkpoint from {ckpt_path}')\n",
...
# ── Cell: RTM Surrogate ──
cells.append(_md(["## 5. Train RTM Surrogate"]))
cells.append(_code([
    ...
    "    ckpt_path = os.path.join(CHECKPOINT_DIR, f'rtm_surrogate_{n_bands}bands.pt')\n",
    "    if os.path.exists(ckpt_path):\n",
    "        print(f'Loading RTM surrogate for {n_bands} bands from checkpoint...')\n",
...
# ── Cell: Attack definitions (inline) ──
cells.append(_md([
...
# ── Cell: Run all attacks ──
cells.append(_md(["## 7. Run All Attacks & Collect Results"]))
cells.append(_code([
    "all_results = []\n",
    ...
    "            scene_norm = cube_norm.transpose(2, 0, 1)  # (n_bands, H, W)\n",
    ...
    "                            atm = atm_state_default.expand(len(test_rows), -1)\n",
    "                            adv_spectra, _ = rtaa.generate(model, pixel_t, test_labels, atm)\n",
...
# ── Cell: Save to Excel ──
cells.append(_md(["## 8. Save Results to Excel"]))
cells.append(_code([
...
# ── Cell: Summary Table ──
cells.append(_md(["## 9. Results Summary"]))
cells.append(_code([
    "# Print summary table\n",
...
# ── Cell: Visualization ──
cells.append(_md(["## 10. Visualize Results"]))
cells.append(_code([
...
print("Notebook generated: notebooks/adversarial_benchmark_colab.ipynb")
