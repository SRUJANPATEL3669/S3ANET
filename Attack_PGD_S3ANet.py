...
def run_pgd(dataID, args):
    ...
    for r in range(args.restarts):
        ...
        adv_image = images.clone().detach() + torch.empty_like(images).uniform_(-args.epsilon, args.epsilon)
        ...
        with torch.no_grad():
            output_final = Model(adv_image)
            final_loss = criterion(output_final, label_tar).item()
    ...
    plt.imsave(save_path_prefix + 'S3ANet_PGD_OA' + repr(int(OA*10000)) +
...
def main(args):
    ...
    for did in dataset_ids:
        print('\n' + '='*55)
        print('  PGD on Dataset: %s' % DataName[did])
        print('='*55)
        try:
            row = run_pgd(did, args)
            results.append(row)
    ...
    if results:
        ...
        excel_path = args.save_path_prefix + 'PGD_All_Metrics_Results.xlsx'
